...
class colourIdentifier():
	...
	def callback(self, data):
		...
		if len(contours1) != 0:
			...
			if colour_max_area < cv2.contourArea(c):
				colour_max_area = cv2.contourArea(c)
				colour = 1
		
		if len(contours2) != 0:
			c1 = max(contours2, key = cv2.contourArea)
			if colour_max_area < cv2.contourArea(c1):
				c = c1
				colour_max_area = cv2.contourArea(c)
				colour = 2
			
		if len(contours3) != 0:
			c1 = max(contours3, key = cv2.contourArea)
			if colour_max_area < cv2.contourArea(c1):
				c = c1
				colour_max_area = cv2.contourArea(c)
				colour = 3
			
		if colour_max_area != 0:
			M = cv2.moments(c)
			cx, cy = int(M['m10']/M['m00']), int(M['m01']/M['m00'])
		...
		print(self.red_visible, self.green_visible, self.blue_visible)
